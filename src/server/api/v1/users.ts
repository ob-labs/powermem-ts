...
export function createUsersRouter(userService: UserService): Router {
  ...
  router.get('/profiles', async (req, res) => {
    try {
      ...
      const result = await userService.listProfiles(limit);
  ...
  router.post('/:userId/profile', async (req, res) => {
    try {
      ...
      const result = await userService.addProfile(req.params.userId, content, metadata, infer ?? true);
      res.json({ success: true, data: result });
    } catch (err) {
      res.status(500).json({ success: false, message: String(err) });
    }
  ...
  router.get('/:userId/memories', async (req, res) => {
    try {
      ...
      const result = await userService.listMemories(req.params.userId, limit, offset);
      res.json({ success: true, data: result });
    } catch (err) {
      res.status(500).json({ success: false, message: String(err) });
    }
  ...
  router.put('/:userId/memories/:memoryId', async (req, res) => {
    try {
      ...
      const result = await userService.updateMemory(req.params.memoryId, content, metadata);
      res.json({ success: true, data: result });
    } catch (err) {
      res.status(500).json({ success: false, message: String(err) });
    }
  ...
  router.delete('/:userId/memories', async (req, res) => {
    try {
      await userService.deleteMemories(req.params.userId);
