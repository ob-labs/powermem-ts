...
export function createMemoriesRouter(memoryService: MemoryService): Router {
  ...
  router.get('/stats', async (req, res) => {
    try {
      ...
      const all = await memoryService.getAll({ userId, agentId, limit: 10000 });
  ...
  router.get('/count', async (req, res) => {
    try {
      const userId = req.query.user_id as string | undefined;
      const agentId = req.query.agent_id as string | undefined;
      const count = await memoryService.count({ userId, agentId });
  ...
  router.get('/users', async (req, res) => {
    try {
      ...
      const users = await memoryService.getUsers(limit);
  ...
  router.get('/export', async (req, res) => {
    try {
      ...
      const memories = await memoryService.exportMemories({ userId, agentId, limit });
  ...
  router.get('/', async (req, res) => {
    try {
      ...
      const result = await memoryService.getAll({ userId, agentId, limit, offset, sortBy, order });
  ...
  router.post('/batch', async (req, res) => {
    try {
      ...
      const result = await memoryService.addBatch(
  ...
  router.post('/import', async (req, res) => {
    try {
      ...
      const result = await memoryService.importMemories(items, { infer: infer ?? false });
  ...
  router.put('/batch', async (req, res) => {
    try {
      ...
      for (const u of updates) {
        const result = await memoryService.update(u.id, u.content ?? '', { metadata: u.metadata });
        results.push(result);
      }
  ...
  router.delete('/batch', async (req, res) => {
    try {
      ...
      let deleted = 0;
      for (const id of ids) {
        if (await memoryService.delete(id)) deleted++;
      }
  ...
  router.get('/:id', async (req, res) => {
    try {
      const result = await memoryService.get(req.params.id);
      if (!result) {
  ...
  router.put('/:id', async (req, res) => {
    try {
      ...
      const result = await memoryService.update(req.params.id, content, { metadata });
  ...
  router.delete('/:id', async (req, res) => {
    try {
      const ok = await memoryService.delete(req.params.id);
